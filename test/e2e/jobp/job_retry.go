...
var _ = Describe("Test job restart on last retry", func() {
	It("last Retry", func() {
		...
		_, err = e2eutil.KubeClient.CoreV1().Pods(job.Namespace).Get(context.TODO(), jobctl.MakePodName(jobName, "failed-task", 0), metav1.GetOptions{})
		Expect(err).NotTo(HaveOccurred())
	})
})
